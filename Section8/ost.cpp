...
OSTNode* kthNodeIn(OSTNode* root, int k) {
    /* Base case: If we walked off the tree,
     * or if we’re looking for an invalid index, we’ve failed.
     */
    if (root == nullptr || k < 0) return nullptr;

    if (k == root->leftSubtreeSize) {
        return root;
    } else if (k > root->leftSubtreeSize + 1) {
        return kthNodeIn(root->right, k - root->leftSubtreeSize);
    } else {
        return kthNodeIn(root->left, root->leftSubtreeSize - k);
    }
    return nullptr;
}
