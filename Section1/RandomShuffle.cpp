...
string randomShuffle(string input) {
    /* Base case: There is only one possible permuation of a string with no charaters in it */
    if (input.empty()) {
        return input;
    }
    ...
    /* Pull that charater to the front, then permute the rest of the string. */
