...
Vector<DataPoint> combineRec(const Vector<Vector<DataPoint>>& sequences, int low, int high);
void merge(const Vector<DataPoint>& left, const Vector<DataPoint>& right,
           Vector<DataPoint>& combination);

/**
 * Function: combine
 * --------------------------------------------------
 * Given a list containing several lists of numbers, each of which is already sorted in order,
 * combine those lists together into one list and return this giant list.
 */
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    return combineRec(sequences, 0, sequences.size() - 1);
}

/**
 * Function: combineRec
 * --------------------------------------------------
 * A helper function using recursion to combine lists in the sequences indexed from low to high.
 */
Vector<DataPoint> combineRec(const Vector<Vector<DataPoint>>& sequences, int low, int high) {
    /* Base case: The sequences vector is empty so the combination result is just an empty vector. */
    if (sequences.isEmpty()) {
        return { };
    }
    /* Base case: There is only one element in the sequences so the combination is just that vector element on its own.*/
    if (low == high) {
        return sequences[low];
    }

    /* Split the sequences into two halves. */
    int middle = (low + high) / 2;
    Vector<DataPoint> left = combineRec(sequences, low, middle);
    Vector<DataPoint> right = combineRec(sequences, middle + 1, high);

    /* Get the combination of each half and then merge them all together. */
    Vector<DataPoint> combination;
 ...
 * Function: merge
 * --------------------------------------------------
 * Merge two sorted lists of numbers into a giant list combination.
 */
void merge(const Vector<DataPoint>& left, const Vector<DataPoint>& right,
           Vector<DataPoint>& combination) {
    int li = 0, ri = 0;
    /* Continuously add the smallest numbers of the two lists until one list is exhausted. */
    while (li < left.size() && ri < right.size()) {
        if (left[li].weight < right[ri].weight) {
            combination.add(left[li]);
            ++li;
        } else {
            combination.add(right[ri]);
            ++ri;
        }
    }

    /* Add all the numbers left into the giant list. */
    while (li < left.size()) {
        combination.add(left[li]);
        ++li;
    }
...
STUDENT_TEST("Merges four lists of different sizes") {
    Vector<DataPoint> one = { { "B" , 2 }};
    ...
    auto merged = combine({ one, two, three, four });
    EXPECT_EQUAL(merged.size(), 7);
    EXPECT_EQUAL(merged[0], two[0]);
    EXPECT_EQUAL(merged[1], one[0]);
    EXPECT_EQUAL(merged[2], two[1]);
    EXPECT_EQUAL(merged[3], three[0]);
    EXPECT_EQUAL(merged[4], two[2]);
    EXPECT_EQUAL(merged[5], four[0]);
    EXPECT_EQUAL(merged[6], three[1]);
}
