...
void getBestSchedule(const Set<Shift>& candidates, const Set<Shift>& scheduleSoFar,
                Set<Shift>& bestSchedule, int valueSoFar, int& maxValue,
                int remainingHours);
bool hasOverlap(const Shift& candidate, const Set<Shift>& schedule);

/**
 * Function: highestValueScheduleFor
 * Usage: highestValueScheduleFor(shifts, maxHours)
 * --------------------------------------------------
 * Given a set of possible shifts and the maximum number of hours the employee is
 * allowed to work each week, return a set of shifts that produce the highest value
 * for the company.
 */
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) error("Number of hours must be greater than or equal to 0.");
    Set<Shift> bestSchedule = {};
    int maxValue = 0;
    getBestSchedule(shifts, {}, bestSchedule, 0, maxValue, maxHours);
    return bestSchedule;
}

/**
 * Function: getBestSchedule
 * Usage: getBestSchedule(candidates, scheduleSoFar, bestSchedule, valueSoFar, maxValue, remainingHours)
 * --------------------------------------------------
 * A helper function to generate bestSchedule with the highest maxValue.
 */
void getBestSchedule(const Set<Shift>& candidates, const Set<Shift>& scheduleSoFar,
                           Set<Shift>& bestSchedule, int valueSoFar, int& maxValue,
                           int remainingHours) {
    /* Update best schedule if there is no more candidate to consider
     * or no remaining hours left. */
    if (candidates.isEmpty() || remainingHours == 0) {
        if (valueSoFar > maxValue) {
            maxValue = valueSoFar;
            bestSchedule.clear();
            for (Shift s: scheduleSoFar) {
                bestSchedule.add(s);
            }
        }
    } else {
        /* Take a random candidate out of the set by simply taking the first item. */
        Shift cand = candidates.first();
        /* Option 1: Includes the candidate in the schedule.
         * If the candidate is not overlap with any shift in the schedule so far and
         * the remaining time is enough to do the shift.*/
        if (lengthOf(cand) <= remainingHours && !hasOverlap(cand, scheduleSoFar)) {
                getBestSchedule(candidates - cand, scheduleSoFar + cand, bestSchedule,
                                valueSoFar + valueOf(cand), maxValue,
                                remainingHours - lengthOf(cand));
        }
        /* Option 2: Not include the candidate in the schedule. */
        getBestSchedule(candidates - cand, scheduleSoFar, bestSchedule,
                        valueSoFar, maxValue, remainingHours);
    }
}

/**
 * Function: hasOverlap
 * Usage: hasOverlap(candidate, schedule)
 * --------------------------------------------------
 * Checks whether the given candidate is overlap
 * with at least one of the shift in the schedule.
 */
bool hasOverlap(const Shift& candidate, const Set<Shift>& schedule) {
    for (Shift shift: schedule) {
        if (overlapsWith(candidate, shift)) return true;
    }
    return false;
}
...
STUDENT_TEST("Lucrative Sunday.") {
    Vector<Shift> shifts = {
        { Day::SUNDAY,  8, 19, 1000},
        { Day::MONDAY,  8, 10, 44 },
        { Day::TUESDAY,  8, 10, 48 },
        { Day::WEDNESDAY,  8, 10, 20 },
        { Day::FRIDAY, 8, 10, 40 },
        { Day::THURSDAY, 8, 10, 40 },
        { Day::SATURDAY, 8, 10, 66 },
    };
    Set<Shift> answer = highestValueScheduleFor(asSet(shifts), 13);
    EXPECT_EQUAL(answer, { shifts[0], shifts[6] });
}
