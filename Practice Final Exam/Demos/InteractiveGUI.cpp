/* Interactive environment for testing the leapfrog hash table. */
...
GRAPHICS_HANDLER("Interactive Leapfrog Hashing", GWindow& window) {
    return make_shared<InteractiveGUI<LeapfrogHashTable>>(window, "LeapfrogHashTable");
}
...
namespace {
    ...
    template <> struct HashTraits<LeapfrogHashTable> {
        static string name() { return "LeapfrogHashTable"; }
    };
...
CONSOLE_HANDLER("Interactive Leapfrog Hashing") {
    demoTable<LeapfrogHashTable>();
}
