...
HeapPQueue::HeapPQueue() {
    /* The heap is one-indexed so the slot 0 in the elems array
     * will never be read from or written to. */
...
void HeapPQueue::enqueue(const DataPoint& data) {
    /* Double the size if the queue is full. */
...
/**
 * Repeats these steps until the current index is top of the heap or its parent has a lower weight:
 *      1. Swap the node in current index with its parent node.
 *      2. Bubble up the parent node.
 */
void HeapPQueue::bubbleUp(int index) {
    ...
    if (elems[index].weight < elems[parentIndex].weight) {
        swap(elems[index], elems[parentIndex]);
...
/**
 * Repeats these steps until the current node has no child or its children have a higher weight:
 *      1. Swap the node in current index with its smaller child node.
 *      2. Bubble down the child node.
 */
void HeapPQueue::bubbleDown(int index) {
    ...
    if (child1Index > logicalSize && child2Index > logicalSize) {
        return;
    }
    /* Case 2: One child. */
    if (child1Index <= logicalSize && child2Index > logicalSize) {
        if (elems[index].weight > elems[child1Index].weight) {
            swap(elems[index], elems[child1Index]);
            bubbleDown(child1Index);
        }
    }
    /* Case 3: Two children. */
    if (child1Index <= logicalSize && child2Index <= logicalSize) {
        int smallerIndex = elems[child1Index].weight < elems[child2Index].weight
                            ? child1Index : child2Index;
        if (elems[index].weight > elems[smallerIndex].weight) {
            swap(elems[index], elems[smallerIndex]);
            bubbleDown(smallerIndex);
        }
    }
}

void HeapPQueue::resize(int capacity) {
    if (capacity < logicalSize + 1) {
        error("The new capacity must be greater than or equal to the current logical size.");
    }
    ...
    DataPoint* newArr = new DataPoint[capacity];
...
/* This function displays the priority queue as an array.
 */
void HeapPQueue::printDebugInfo() {
    for (int i = 1; i <= logicalSize; ++i) {
        cout << "[ " << elems[i].name << " (" << elems[i].weight << ") " << "] ";
    }
    cout << endl;
}
...
STUDENT_TEST("Question 4 and 5.") {
    ...
    /* We should have
     *
     * [ unused ] [ T ] [ B ] [ G ] [ S ] [ A ] [ V ] [ R ] [ O ] [ K ]
     *
     */
    EXPECT_EQUAL(pq.size(), 9);
    EXPECT_EQUAL(pq.elems[1], T);
    EXPECT_EQUAL(pq.elems[2], B);
    EXPECT_EQUAL(pq.elems[3], G);
    EXPECT_EQUAL(pq.elems[4], S);
    EXPECT_EQUAL(pq.elems[5], A);
    EXPECT_EQUAL(pq.elems[6], V);
    EXPECT_EQUAL(pq.elems[7], R);
    EXPECT_EQUAL(pq.elems[8], O);
    EXPECT_EQUAL(pq.elems[9], K);

    pq.dequeue();
    /* We should have
     *
     * [ unused ] [ G ] [ B ] [ R ] [ S ] [ A ] [ V ] [ K ] [ O ]
     *
     */
    EXPECT_EQUAL(pq.size(), 8);
    EXPECT_EQUAL(pq.elems[1], G);
    EXPECT_EQUAL(pq.elems[2], B);
    EXPECT_EQUAL(pq.elems[3], R);
    EXPECT_EQUAL(pq.elems[4], S);
    EXPECT_EQUAL(pq.elems[5], A);
    EXPECT_EQUAL(pq.elems[6], V);
    EXPECT_EQUAL(pq.elems[7], K);
    EXPECT_EQUAL(pq.elems[8], O);
}
