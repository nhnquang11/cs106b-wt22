...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if (populations.size() > numSeats) {
        error("The number of seats must be greater than or equal to the number of states.");
    }

    Map<string, int> seats;
    HeapPQueue pq;

    /* Grant an initial seat to each state. */
    for (string state: populations) {
        seats[state] = 1;
        /* Because the Huntington-Hill algorithm finds the entry with the highest weight and
         * the HeapPQueue is excellent at finding the entry with the lowest weight, here we
         * reverse the sign of the states' weight when enqueuing.
         */
        double weight = - populations[state] / sqrt(seats[state] * (seats[state] + 1));
        DataPoint node = { state, weight };
        pq.enqueue(node);
        --numSeats;
    }

    /* Grant the seats left using Huntington-Hill algorithm. */
    while (numSeats > 0) {
        DataPoint node = pq.dequeue();
        string state = node.name;
        ++seats[state];
        double newWeight = - populations[state] / sqrt(seats[state] * (seats[state] + 1));
...
STUDENT_TEST("H is the best.") {
    ...
    EXPECT_EQUAL(apportion(populations, 10), seats);
}
