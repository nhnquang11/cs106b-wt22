...
private:
    ...
    /** A helper function scanning forward from position i to insert elem into the table. */
    bool insert(std::string elem, int i, int dist);
    /** Move the slot in position 1 step backward, assuming that the previous slot is empty.*/
