...
bool RobinHoodHashTable::insert(string elem, int i, int dist) {
    /* Scan forward – wrapping around if necessary – keeping track of the number of steps taken.
     * If finding an empty slot, place the element there. Otherwise, if the current slot is full and
     * closer to home than the element to be inserted, place the item to insert there, displacing the
     * element that was at that spot, and continue the insertion for the displaced element. */
    while (dist < numSlots) {
        /* A blank slot. */
        if (elems[i].distance == EMPTY_SLOT) {
            elems[i].distance = dist;
            elems[i].value = elem;
            return true;
        }
        /* Elem is already in the table. No need to insert. */
        if (elems[i].value == elem) {
            return false;
        }
        /* A filled slot closer to home than the number of steps taken. */
        if (elems[i].distance < dist) {
            int distToInsert = elems[i].distance;
            string elemToInsert = elems[i].value;

            /* Place the item to insert in this position. */
            elems[i].value = elem;
            elems[i].distance = dist;

            /* Update elem and dist to insert the item just displaced. */
            elem = elemToInsert;
            dist = distToInsert;
        }
        ++dist;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int i = hashCode;
    int dist = 0;
    /* Scan forward – wrapping around if necessary – keeping track of how many steps taken.
     * Stop when finding the item, a blank slot, or a filled slot closer to home than the number of
     * steps taken. */
    while (dist < numSlots) {
        /* A blank slot. */
...
void RobinHoodHashTable::moveBackward(int i) {
    /* Keep moving forward – wrapping around as necessary – moving elements backward in the table
     * one slot until an empty slot or an item in its home position is found. */
    while (!(elems[i].distance == EMPTY_SLOT || elems[i].distance == 0)) {
        int prevIndex = (i + numSlots - 1) % numSlots;
        if (elems[prevIndex].distance != EMPTY_SLOT) {
            error("Can not implement backward shifting. The previous slot is filled.");
        }
        /* Pass the information of the current slot to the previous slot. */
        elems[prevIndex].distance = elems[i].distance - 1;
        elems[prevIndex].value = elems[i].value;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int i = hashCode;
    int dist = 0;
    /* Walk forward – wrapping around if necessary – until the item or an empty slot is found.
     * If the item is found, remove it. Then, keep moving forward – wrapping around as necessary –
     * moving elements backward in the table one slot until an empty slot or an item in its
