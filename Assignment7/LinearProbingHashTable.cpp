...
bool LinearProbingHashTable::insert(const string& elem) {
    /* Can not insert elem anymore. The table is full. */
    if (size() == numSlots) {
        return false;
    }
    ...
    /* Keep track of the current position in the walking forward process. */
    int i = hashCode;
    /* Walk forward – wrapping around if necessary – until a blank spot or tombstone slot is found.
     * Then, put the item there. */
    while (true) {
        /* Slot is not filled so we can insert elem here. */
        if (elems[i].type == SlotType::EMPTY || elems[i].type == SlotType::TOMBSTONE) {
            /* Check forward to avoid duplicate elements. */
            if (contains(elem, i + 1, (hashCode + numSlots - 1) % numSlots)) {
                return false;
            }
            /* If the table doesn't contain the element, insert the element to the current position. */
            elems[i].value = elem;
...
bool LinearProbingHashTable::contains(const std::string& elem, int start, int end) const {
    if (isEmpty()) return false;

    /* Keep track of the current position in the scanning forward process. */
    int i = start;
    ...
    while (i != end) {
        ...
        /* Using modulo to avoid out of bound error. */
        i = (i + 1) % numSlots;
    }

    /* Check one more time due to the fencepost problem. */
    if (elems[i].type == SlotType::FILLED && elems[i].value == elem) {
        return true;
    }
    return false;
...
bool LinearProbingHashTable::contains(const std::string& elem, int hashCode) const {
    return contains(elem, hashCode, (hashCode + numSlots - 1) % numSlots);
}

bool LinearProbingHashTable::contains(const string& elem) const {
    if (isEmpty()) return false;
    int hashCode = hashFn(elem);
    return contains(elem, hashCode);
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    /* Check the current position before the loop to handle the fencepost problem. */
    if (elems[i].type == SlotType::FILLED && elems[i].value == elem) {
     ...
     * If the item is found, replace it with a tombstone. */
    while (i != hashCode) {
        if (elems[i].type == SlotType::FILLED && elems[i].value == elem) {
            elems[i].type = SlotType::TOMBSTONE;
            elems[i].value = "";
