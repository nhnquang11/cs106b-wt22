...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork, int numCitiesLeft,
                               Set<string>& supplyLocations, const Set<string>& locationsRemaining);
bool makeStockpilingLocation(const string& location, const Map<string, Set<string>>& roadNetwork,
                             int numCitiesLeft, Set<string>& supplyLocations,
                             const Set<string>& locationsRemaining);

/**
 * Function: canBeMadeDisasterReady
 * --------------------------------------------------
 * A country is disaster-ready if every city either already has emergency supplies
 * or is immediately down the highway from  a city that has them.
 *
 * Given a map marking the road network, a number of cities you can afford to put supplies in,
 * returns true if a solution exists and filled in an outparameter with which cities to choose
 * to be a stockpiling supply.
 */
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
    ...
    /* Initialize a set containing all the locations to use the helper function. */
    Set<string> locationsRemaining;
    for (string location: roadNetwork) {
        locationsRemaining.add(location);
    }

    return canBeMadeDisasterReadyRec(roadNetwork, numCities, supplyLocations, locationsRemaining);
}

/**
 * Function: canBeMadeDisasterReadyRec
 * --------------------------------------------------
 * A helper funciton using recursion to find the solution.
 */
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                               int numCitiesLeft, Set<string>& supplyLocations,
                               const Set<string>& locationsRemaining) {
    /* All the locations are ready for a disaster. */
    if (locationsRemaining.isEmpty()) {
        return true;
    }

    /* There is no city left available and there still exists a location that is not ready for a disaster. */
    if (numCitiesLeft == 0) {
        return false;
    }

    /* Take a random location yet ready out. */
    string takenOut = locationsRemaining.first();
    /* Option 1: Make this location a stockpiling supply location. */
    if (makeStockpilingLocation(takenOut, roadNetwork, numCitiesLeft, supplyLocations, locationsRemaining)) {
            return true;
    }

    /* Option 2: Make one of this location's adjacent neighbours a stockpiling supply location. */
    for (string neighbour: roadNetwork[takenOut]) {
        if (makeStockpilingLocation(neighbour, roadNetwork, numCitiesLeft, supplyLocations, locationsRemaining)) {
                return true;
        }
    }
    return false;
...
/**
 * Function: makeStockpilingLocation
 * --------------------------------------------------
 * Using backtracking and mutually recursion with the function canBeMadeDisasterReadyRec
 * to make a designated location an emergency supply.
 */
bool makeStockpilingLocation(const string& location, const Map<string, Set<string>>& roadNetwork,
                             int numCitiesLeft, Set<string>& supplyLocations,
                             const Set<string>& locationsRemaining) {
    supplyLocations.add(location);
    Set<string> newLocationsRemaining = locationsRemaining - location;
    for (string neighbour: roadNetwork[location]) {
        newLocationsRemaining -= neighbour;
    }
    if (canBeMadeDisasterReadyRec(roadNetwork, numCitiesLeft - 1, supplyLocations, newLocationsRemaining)) {
        return true;
    }
    supplyLocations -= location;
    return false;
}
...
STUDENT_TEST("Superstar city.") {
     ...
     *
     */
    Vector<string> cities = { "1", "2", "3", "4", "5", "6", "7", "8", "9" };

    Map<string, Set<string>> map = makeSymmetric({
        { "5", { "1", "2", "3", "4", "6", "7", "8", "9" } },
    });

    /* We should be able to cover everything with only one city: 5
     */
    Set<string> chosen;
