...
breakWeakPasswords(const Map<string, int>& stolenPasswordFile,
    ...
    Map<int, string> hashCodeToWeakPassword;
    for (string password: knownWeakPasswords) {
        int hashCode = hashFn(password);
        hashCodeToWeakPassword[hashCode] = password;
    }

    Map<string, string> userToPassword;
    for (string user: stolenPasswordFile) {
        int hashCode = stolenPasswordFile[user];
        if (hashCodeToWeakPassword.containsKey(hashCode)) {
            userToPassword[user] = hashCodeToWeakPassword[hashCode];
        }
    }
