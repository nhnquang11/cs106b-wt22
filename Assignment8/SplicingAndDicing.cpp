...
string fromDNA(Nucleotide* dna) {
    /* Return an empty string if the dna sequence is empty. */
    if (dna == nullptr) return "";
    string result = "";
    /* Traverse through the dna sequence and add each nucleotide to the result. */
    while (dna != nullptr) {
        result += dna->value;
        dna = dna->next;
    }
...
Nucleotide* toStrand(const string& str) {
    ...
    /* Create a dummy cell as a cell just before the first cell of the strand. */
    Nucleotide* dummy = new Nucleotide;
    dummy->prev = dummy->next = nullptr;
    /* Create a pointer to keep track of the next position to attach a nucleotide. */
    Nucleotide* ptr = dummy;
    for (char nuc: str) {
    ...
    /* Create a head pointer pointing to the first nuc in the strand and free the memory of the dummy cell. */
    Nucleotide* head = dummy->next;
    head->prev = nullptr;
    delete dummy;
    return head;
}
...
/**
 * Check whether strand a is the prefix of strand b
 *
 * E.g.
 *  >>> a = "ATG"
 *  >>> b = "ATGXXXGTA"
 ...
 *  false
 */
bool isPrefix(Nucleotide* a, Nucleotide* b) {
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    while (dna != nullptr) {
        if (isPrefix(target, dna)) return dna;
        dna = dna->next;
    }
    return nullptr;
}
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    Nucleotide* toRemove = findFirst(dna, target);
    if (toRemove == nullptr) return target == nullptr;

    /* Attach a dummy cell to the beginning of the strand. */
    Nucleotide* dummy = new Nucleotide;
    dummy->next = dna;
    dummy->prev = nullptr;
    dna->prev = dummy;
    ...
    /* Update dna pointing to the next cell after the dummy and free the memory of the dummy cell. */
    dna = dummy->next;
    if (dna != nullptr) {
        dna->prev = nullptr;
    }
...
STUDENT_TEST("deleteNucleotides cleans up an one-element sequence.") {
...
STUDENT_TEST("fromDNA handles two-character strings.") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->prev = nullptr;
...
STUDENT_TEST("findFirst works when target not present.") {
    Nucleotide* dna    = toStrand("ATGXATGX");
    Nucleotide* target = toStrand("ATGXATXG");
...
STUDENT_TEST("spliceFirst works at the beginning of a strand.") {
    Nucleotide* dna    = toStrand("AG");
